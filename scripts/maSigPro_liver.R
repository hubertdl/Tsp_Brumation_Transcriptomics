# =============================================================================
# maSigPro Time-Course Analysis — Liver
# =============================================================================
...
#
# Description:
#   Time-course differential expression analysis of liver RNA-seq data across
...
#   raw_countsL.tab  — raw counts matrix (transcripts x samples), tab-delimited,
#                      first column is transcript ID
#   Lkey.tab         — maSigPro edesign key file with columns:
#                      Time, Replicate, <group columns as required by maSigPro>
#
# Output files:
#   checkpoints/fit_liver.rds     — saved p.vector() output (re-used on re-runs)
#   checkpoints/tstep_liver.rds   — saved T.fit() output  (re-used on re-runs)
#   k18/k18_cluster1.tab ...      — gene lists per cluster
#   silhouette_liver.png          — cluster number selection plot
#
# Packages required:
...
rawcounts_dt           <- fread("raw_countsL.tab", data.table = FALSE)
...
if (file.exists("checkpoints/fit_liver.rds")) {
  ...
  fit <- p.vector(data    = data,
                  ...
                  counts  = TRUE,
                  min.obs = MIN_OBS)
  saveRDS(fit, "checkpoints/fit_liver.rds")
}
cat("  Significant transcripts:", fit$i, "\n")

# ── 7. T.fit — stepwise regression (slow; checkpointed) ──────────────────────

if (file.exists("checkpoints/tstep_liver.rds")) {
  cat("Loading saved T.fit output...\n")
  tstep <- readRDS("checkpoints/tstep_liver.rds")
} else {
  cat("Running T.fit()...\n")
  tstep <- T.fit(fit)
  saveRDS(tstep, "checkpoints/tstep_liver.rds")
}
...
png("silhouette_liver.png", width = 1800, height = 1400, res = 200)
...
plot(K_EVAL_RANGE, sil_scores, type = "b", pch = 19,
     ...
     main = "Cluster number selection — Liver maSigPro analysis")
...
par(mfrow = c(1, 1))
dev.off()
cat("  Silhouette plot saved to silhouette_liver.png\n")
